...
from utils.stream import SteamGenerationMixin, printf
...
parser.add_argument("--lora_path", type=str, default="/workspace/checkpoints/llama-l7b-lora/checkpoint-10200")
parser.add_argument("--use_typewriter", type=int, default=1)
parser.add_argument("--share_link", type=int, default=0)
parser.add_argument("--use_local", type=int, default=1)
args = parser.parse_args()

tokenizer = LlamaTokenizer.from_pretrained(args.model_path, cache_dir="/workspace/checkpoints")
...
def generate_prompt_and_tokenize0(data_point, maxlen):
    ...
    if seqlen > maxlen:
        raise Exception(
            '>>> The input question is too long! Cosidering increase the Max Memory value or decrease the length of input! ')
    history_prompt = ''
        ...
        with gr.Column(variant="panel"):
            ...
            with input_component_column:
                ...
                type_of_prompt = gr.components.Dropdown(
                    ['0', '1'], value='1', label="Prompt Type",
                    info="select the specific prompt; use after clear history"
                )
                input_components = [
                    input, history, temperature, topp, topk, beam_number, max_new_token, min_new_token, repeat_penal,
                    max_memory, do_sample, type_of_prompt
                ]
                input_components_except_states = [input, temperature, topp, topk, beam_number, max_new_token,
                                                  min_new_token, repeat_penal, max_memory, do_sample, type_of_prompt]
            with gr.Row():
...
demo.queue().launch(share=args.share_link != 0, inbrowser=True, server_name="0.0.0.0", server_port=7860)
