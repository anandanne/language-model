...
with open("/workspace/demo/assets/custom.css", "r", encoding="utf-8") as f:
    customCSS = f.read()
...
with gr.Blocks(css=customCSS, theme=small_and_beautiful_theme) as demo:
            ...
            with gr.Column(min_width=50, scale=1):
                with gr.Tab(label="模型"):
                    ...
                    model_select_dropdown = gr.Dropdown(
                        label="选择模型", choices=MODELS, multiselect=False, value=MODELS[DEFAULT_MODEL],
                        interactive=True
                    )
    ...
    model_select_dropdown.change(current_model.get_model,
                                 [model_select_dropdown, lora_select_dropdown, keyTxt, temperature_slider, top_p_slider,
                                  systemPromptTxt], [status_display, lora_select_dropdown], show_progress=True)
    lora_select_dropdown.change(current_model.get_model,
                                [model_select_dropdown, lora_select_dropdown, keyTxt, temperature_slider, top_p_slider,
                                 systemPromptTxt], [status_display], show_progress=True)
...
if __name__ == "__main__":
    ...
    if dockerflag:
        if authflag:
            demo.queue(concurrency_count=CONCURRENT_COUNT).launch(
                ...
                server_port=7860,
                auth=auth_list,
                favicon_path="/workspace/demo/assets/favicon.ico",
            )
        else:
            demo.queue(concurrency_count=CONCURRENT_COUNT).launch(
                server_name="0.0.0.0",
    ...
    # if not running in Docker
        ...
        if authflag:
            ...
            demo.queue(concurrency_count=CONCURRENT_COUNT).launch(
                share=False, favicon_path="/workspace/demo/assets/favicon.ico", inbrowser=True,
